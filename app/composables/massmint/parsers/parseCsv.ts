...
function processCsvLines(lines: string[], csvHeaders: string[]): Record<string, Entry> {
  ...
  for (const line of lines) {
    ...
    for (const [j, header] of csvHeaders.entries()) {
      ...
      if (header === 'price') {
        entry.price = value ? Number.parseFloat(value) : undefined
      }
        ...
        entry[header as keyof Entry] = value as any
      }
    }
...
function parseCsvWithHeaders(lines: string[], csvHeaders: string[]): Record<string, Entry> {
  return processCsvLines(lines, csvHeaders)
}

function parseCsvWithoutHeaders(lines: string[]): Record<string, Entry> {
