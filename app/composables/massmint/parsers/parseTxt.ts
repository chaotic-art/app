...
function parsePriceAndCurrency(fieldValue: string): {
  ...
  if (priceMatch) {
    const price = Number.parseFloat(priceMatch.groups?.price?.replace(',', '') || '')
...
function updateEntry(entry: Record<string, unknown>, line: string) {
...
function updateEntries(entries: Record<string, Entry>, block: string) {
