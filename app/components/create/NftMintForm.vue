...
const props = withDefaults(
  ...
  {
    title: 'Create NFT',
    description: 'Create a unique digital asset or artwork as an NFT',
    submitLabel: 'Create NFT',
  },
)

const options = computed(() => ({
  fixedCollectionId: props.fixedCollectionId,
  fixedBlockchain: props.fixedBlockchain,
}))
...
const fixedBlockchainLabel = computed(() => {
  if (!isBlockchainFixed.value || !state.blockchain)
    return ''
  return blockchains.find(b => b.value === state.blockchain)?.label ?? state.blockchain
})

watchDebounced(
  [isWalletConnected, mediaFile, () => state.collection, () => state.name, () => state.description, () => state.supply, () => validProperties.value.length],
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div v-if="!isBlockchainFixed" class="w-full">
                <UFormField
                  name="blockchain"
                  label="Blockchain"
                  required
              ...
              <div v-else class="w-full">
                <UFormField
                  name="blockchain"
                  label="Blockchain"
                  help="Blockchain for this collection"
                >
                  <UInput
                    :model-value="fixedBlockchainLabel"
                    disabled
                    class="w-full"
                  />
                </UFormField>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div v-if="!isCollectionFixed" class="w-full">
                <UFormField
                  name="collection"
                  label="Collection"
                  required
              ...
              <!-- Selected Collection Info (show when collection is selected or fixed) -->
              <div v-if="selectedCollection" class="flex flex-col gap-1 w-full">
                <h4 class="text-sm font-medium text-gray-900 dark:text-white">
                  {{ isCollectionFixed ? 'Collection' : 'Selected Collection' }}
                </h4>
