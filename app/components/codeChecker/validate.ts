...
interface HtmlContentValidationResult {
...
function validateCanvasCreation(sketchFileContent: string): Result<RegExpExecArray> {
...
function validateGetURLParamsUsage(sketchFileContent: string): Result<RegExpExecArray> {
...
function validateURLSearchParamsUsage(sketchFileContent: string): Result<RegExpExecArray> {
...
function validateURLParamsUsage(sketchFileContent: string): Result<RegExpExecArray> {
...
function doesNotUseExternalResources(htmlFileContent: string): boolean {
  const doc = getDocumentFromString(htmlFileContent)
  const extenal: boolean[] = []

  Object.keys(AssetElementMap).forEach((asset) => {
    const assetConfig = AssetElementMap[asset as keyof typeof AssetElementMap]
    doc.querySelectorAll(assetConfig.tag).forEach((tag) => {
      extenal.push(
        !(tag as any)[assetConfig.src].includes(window.location.hostname),
      )
    })
  })
...
function validateHtmlContent(htmlFileContent: string): HtmlContentValidationResult {
...
function validateSketchContent(sketchFileContent: string, canvasMatch: RegExpExecArray): Pick<
  Validity,
  | 'canvasSize'
  | 'localP5jsUsed'
  | 'validTitle'
...
export function validate(htmlFileContent: string, sketchFileContent: string): Result<InnerValidity> {
